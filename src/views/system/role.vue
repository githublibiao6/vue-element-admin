    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'编辑权限':'新增角色'">
      <el-form :model="role" label-width="80px" label-position="left">
        ...
        <el-form-item label="菜单">
          <el-tree ref="tree" :check-strictly="checkStrictly" :data="routesData" :props="defaultProps" show-checkbox node-key="menuId" class="permission-tree" />
        </el-form-item>
      </el-form>
...
import { getRoutes, getRoles, addRole, deleteRole, updateRole, getMenuByRoleId } from '@/api/role'
...
export default {
  ...
  methods: {
    ...
    getMenuByRoleId() {
      getMenuByRoleId(this.role.id).then(res => {
    ...
    async confirmRole() {
      ...
      for (let index = 0; index < checkedNodes.length; index++) {
        checkedKeys.push(checkedNodes[index].menuId)
      }
      this.role.menus = checkedKeys.join(',')
